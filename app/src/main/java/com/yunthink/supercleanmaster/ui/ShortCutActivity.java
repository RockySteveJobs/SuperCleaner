...
public class ShortCutActivity extends BaseActivity implements CoreService.OnPeocessActionListener {
    ...
    public void onCleanCompleted(Context context, long cacheSize) {
        if (cacheSize > 0) {
            T.showLong(mContext, ShortCutActivity.this.getString(R.string.app_name) + StorageUtil.convertStorage(cacheSize) +  ShortCutActivity.this.getString(R.string.memory));
        } else {
            T.showLong(mContext, ShortCutActivity.this.getString(R.string.no_clean));
        }
